	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server := httptest.NewServer(rest.Handler(func(w http.ResponseWriter, r *http.Request) {
				...
				if !reflect.DeepEqual(receiver.Usr, tt.args.newUser) {
					t.Errorf("body does not match request \n%v\n!=\n%v", receiver.Usr, tt.args.newUser)
				}
			...
			if err := c.CreateUser(tt.args.ctx, tt.args.uid, tt.args.newUser); (err != nil) != tt.wantErr {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server := httptest.NewServer(rest.Handler(func(w http.ResponseWriter, r *http.Request) {
				...
				if receiver.UsrToBeDeleted != tt.args.usrToBeDeleted {
					t.Errorf("user to be deleted does not match \n%v\n!=\n%v", receiver.UsrToBeDeleted, tt.args.usrToBeDeleted)
				}
			...
			if err := c.DeleteUser(tt.args.ctx, tt.args.uid, tt.args.usrToBeDeleted); (err != nil) != tt.wantErr {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server := httptest.NewServer(rest.Handler(func(w http.ResponseWriter, r *http.Request) {
				...
				if receiver.NewPwd != tt.args.newPwd {
					t.Errorf("new password does not match \n%v\n!=\n%v", receiver.NewPwd, tt.args.newPwd)
				}
				...
				if receiver.UsrToBeUpdated != tt.args.usrToBeUpdated {
					t.Errorf("user to be updated does not match \n%v\n!=\n%v", receiver.UsrToBeUpdated, tt.args.usrToBeUpdated)
				}
			...
			if err := c.UpdatePassword(tt.args.ctx, tt.args.uid, tt.args.usrToBeUpdated, tt.args.newPwd); (err != nil) != tt.wantErr {
