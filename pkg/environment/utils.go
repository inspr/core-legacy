...
func IsInChannelBoundary(
	channel string,
	boundary []brokers.ChannelBroker,
) bool {
...
func GetResolvedBoundaryChannelList(
	channels []brokers.ChannelBroker,
) utils.StringArray {
...
func GetSchema(channel string) (string, error) {
	...
	if !ok {
		return "", ierrors.New("schema for channel %s not found", channel).
			NotFound()
	}
	return schema, nil
}
...
func OutputBrokerChannnels(broker string) utils.StringArray {
	return GetChannelBoundaryList(
		GetOutputChannelsData(),
...
func InputBrokerChannels(broker string) utils.StringArray {
	return GetChannelBoundaryList(
		GetInputChannelsData(),
	) // WRONG IMPLEMENTATION
}

// GetResolvedChannel gets a resolved channel from a channel name
func GetResolvedChannel(
	channel string,
	inputChan, outputChan []brokers.ChannelBroker,
) (string, error) {
	if IsInChannelBoundary(channel, inputChan) ||
		IsInChannelBoundary(channel, outputChan) {
