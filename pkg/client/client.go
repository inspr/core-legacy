...
func init() {
	logger, alevel = logs.Logger(
		zap.Fields(
			zap.String("section", "sidecar-client"),
			zap.String("dapp-name", os.Getenv("INSPR_APP_ID")),
		),
	)
}
...
func NewAppClient() *Client {
	...
	logger = logger.With(
		zap.String("read-address", readAddr),
		zap.String("write-address", writeAddr),
	)
	return &Client{
		readAddr: readAddr,
...
func (c *Client) HandleChannel(
	channel string,
	handler func(ctx context.Context, body io.Reader) error,
) {
...
func (c *Client) Run(ctx context.Context) error {
	...
	logger.Info(
		"inspr client server is running",
		zap.String("log-level", alevel.String()),
	)
