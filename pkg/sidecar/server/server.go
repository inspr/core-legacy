...
func (s *Server) Init(r models.Reader, w models.Writer) {
	...
	s.writeAddr = fmt.Sprintf(":%s", os.Getenv("INSPR_SIDECAR_KAFKA_WRITE_PORT"))
	s.client = request.NewJSONClient(fmt.Sprintf("http://localhost:%v", os.Getenv("INSPR_SIDECAR_KAFKA_READ_PORT")))
