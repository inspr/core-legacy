...
class ChannelClient(controller.ControllerClient):
    def get(self, scope:str, key:str) -> dict:
        ...
        try:
            resp = rest.send_get_request(self.url + "/" + ALIAS_ROUTE, body=msg_body, headers=headers)
    ...
    def delele(self, scope:str, key:str, dryRun:bool) -> dict:
        ...
        try:
            resp = rest.send_delete_request(self.url + "/" + ALIAS_ROUTE, body=msg_body, headers=headers)
            print(resp, file=sys.stderr)
            return resp
        except Exception as e:
    ...
    def create(self, scope:str, target:str, alias:dict, dryRun:bool) -> dict:
        ...
        try:
            resp = rest.send_post_request(self.url + "/" + ALIAS_ROUTE, msg_body, headers)
            print(resp, file=sys.stderr)
            return resp
        except Exception as e:
    ...
    def update(self, scope:str, target:str, alias:dict, dryRun:bool) -> dict:
        ...
        try:
            resp = rest.send_update_request(self.url + "/" + ALIAS_ROUTE, msg_body, headers)
            print(resp, file=sys.stderr)
            return resp
        except Exception as e:
